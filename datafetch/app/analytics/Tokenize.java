...
public class Tokenize {
    ...
    public static String happyEmoticon = OR(
            "(?:>|&gt;)?" + OR(normalEyes, wink) + OR(noseArea,"[Oo]") +
                    OR(tongue, happyMouths),

            "(?<=(?: |^))" + OR(happyMouths) + noseArea + OR(normalEyes, wink) + "(?:<|&lt;)?"
    );

    public static String sadEmoticon = OR(
            "(?:>|&gt;)?" + OR(normalEyes, wink) + OR(noseArea,"[Oo]") +
                    OR(tongue, sadMouths),

            "(?<=(?: |^))" + OR(sadMouths) + noseArea + OR(normalEyes, wink) + "(?:<|&lt;)?"
    ...
    static Pattern emoticons = Pattern.compile(emoticon);
    static Pattern hashtags = Pattern.compile(Hashtag);
    static Pattern users = Pattern.compile(AtMention);
    static Pattern happyEmoticons = Pattern.compile(happyEmoticon);
    static Pattern sadEmoticons = Pattern.compile(sadEmoticon);
    static Pattern urls = Pattern.compile(url);
    ...
    public static void main(String[] args) throws IOException {
        ...
        while ( (line = input.readLine()) != null) {
            ...
            for (int i=0; i<toks.size(); i++) {
                //System.out.println(emoticons.matcher(toks.get(i)).matches());
                String token = toks.get(i);
                if (happyEmoticons.matcher(token).matches()) System.out.print("#happy");
                else if (sadEmoticons.matcher(token).matches()) System.out.print("#sad");
                else if (hashtags.matcher(token).matches()) System.out.print("#hashtag");
                else if (users.matcher(token).matches()) System.out.print("#user");
                else if (urls.matcher(token).matches()) System.out.print("#url");
                else {
